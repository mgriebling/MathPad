...
-(NSString *)stringForObjectValue:(id)coordinateValue
{
    ...
    if ( [coordinateValue respondsToSelector:@selector(doubleValue)] ) {
        ...
        if ( fraction == 0.0 ) {
            ...
            string = [NSString stringWithFormat:@"%gπ", fraction];
        }
        else if ( divisor != denominator ) {
            string = [NSString stringWithFormat:@"%gπ/%g", numerator / divisor, denominator / divisor];
        }
        else {
            string = [NSString stringWithFormat:@"%gπ/%g", numerator, denominator];
        }
    }
