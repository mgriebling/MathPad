...
static char digSep;
static char fracSep;
static msAngle angleType;
static fpFormat numberFormat;
static int numberBase;

void Complex::InitSettings()
{
	decPoint = 0;
	numDigits = Real::maxDigits;
	digSep = 0;
	fracSep = 0;
	angleType = Degrees;
	numberFormat = Normal;
...
void Complex::SetDecPoint (int dp)
{
	decPoint = dp;
}

void Complex::SetDigits (int digits)
{
	numDigits = digits;
...
static void Insert (char *res, const char *str, char *pos) {
	char Working[strlen(res)];
	strncpy(Working, res, pos-res-1);	// start of string
	strcat(Working, str);				// inserted string
	strcat(Working, pos);				// add remaining string
	strcpy(res, Working);
}


void Complex::NumToStr(const Real& A, short Decimal, short Digits, char *Str, char Mode)
{
	char s[1024];
	long ds = 3;
	if (GetBase() == 10)
	...
	cs[1] = 0x0;
	dp = strrchr(Str, '.');  // Str.FindFirst('.');
	if (dp == NULL)
		dp = &Str[strlen(Str)-1];  // dp = Str.CountChars();
	else if (GetDigitSep() == '.')
		dp[0] = ',';
	...
	if ((GetDigitSep() != 0x0) && (Mode == Normal)) {
		...
		while (i > &Str[0]) {
			if (GetBase() == 10) {
				if (isdigit(*(i-1)) && isdigit(*i)) Insert(Str, cs, i);
			} else {
				if (isalnum(*(i-1)) && isalnum(*i)) Insert(Str, cs, i);
			}
			i -= ds;
			dp++;
		}
	...
	if (GetFracSep() != 0x0) {
 		exp = strrchr(Str, 'E');  // Str.FindFirst('E');
 		if (exp == NULL) exp = &Str[strlen(Str)-1];
		i = dp + ds + 1;
		cs[0] = GetFracSep();
		while (i < exp) {
			Insert(Str, cs, i);
...
void Complex::ComplexToStr(const Complex& A, char *Str)
{
	bool iZero, rZero;
	short ExpWidth;
	bool EngFormat;
	...
	if (!iZero) {
		...
		if (Real::cmp(Aim, One) != 0) {
			...
			strcat(Str, Result); // Str.Append(Result);
		}
		strcat(Str, "i"); // Str.Append("i");
	}
}
...
{	
	...
	int rexp = A.re.exponent();
	int iexp = A.im.exponent();
